...
export function loadCalApi(embedJsUrl: string = EMBED_JS_URL): CalApi | null {
  ...
  if (!win.Cal) {
    const cal: CalApi = function (...args: unknown[]) {
      ...
      // This mirrors Cal's published snippet: embed.js expects `ns` to exist
      // when it replays the queue, so the branch stays even though nothing
      // here registers a namespace.
      //
      // Namespaces cannot be used to give each booking card its own embed.
      // embed.js upgrades them into real instances in ONE loop that runs once,
      ...
      //
      // A namespace registered after that moment gets a queue and never an
      // instance, so its `inline` call is never processed and no iframe
      // appears; the constructor is module-private, so a late namespace cannot
      // be instantiated. Verified against embed.js v1.6.0 (fingerprint
      // e8a6bde7) and reproduced live: with one namespace per card, the first
      // rendered and every later one showed the fallback. BookingCard solves
      // the collision by owning a single embed instead.
      if (args[0] === 'init') {
