...
- Frontend format check: `cd frontend && npm run format`
- Frontend typecheck: `npm run typecheck` (also `cd frontend && npm run typecheck`)
- Frontend combined check: `cd frontend && npm run validate` (lint + format + typecheck)
- Manual voice pipeline smoke test (not an automated test): `npm run test:voice`

Testing is opt-in. Neither the backend nor the frontend has an automated unit
test runner configured yet. Run `/tests` or `$tests` to add one (pytest for the
backend, Vitest for the frontend) and update this section with the real test
commands.
...
No combined `Verify` command or GitHub Actions check exists yet at the repo
root. `frontend/.github/workflows/validate.yml` runs the frontend's own
lint/format/typecheck on its own, scoped to the frontend directory only. Run
`/ci` or `$ci` to define one project-wide `Verify` command and matching
GitHub check.
